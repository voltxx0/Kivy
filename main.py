...
windowraw = (171, 147, 189, 1)
buttonraw = (107, 105, 138, 1)


Window.clearcolor = tuple(c/255 for c in windowraw[:3]) + (windowraw[3],)
btn_color = tuple(c/255 for c in buttonraw[:3]) + (buttonraw[3],)
...
class PulseScr2(Screen):
  def __init__(self, **kwargs):
      ...
      self.lbl_sec = Seconds(15)
      self.lbl_sec.bind(done=self.sec_finished)
      self.lbl1 = Label(text='Calculate your heart rate')
  ...
  def sec_finished(self, *args):
    if self.lbl_sec.done == True:
     if self.stage == 0:
         # finished the first count, resting
         self.stage = 1
         self.lbl1.text = 'Rest'
         self.lbl_sec.restart(30)
         self.in_result1.set_disabled(False)
     elif self.stage == 1:
         # finished rest, calculating
         self.stage = 2
         self.lbl1.text='Calculate your heart rate'
